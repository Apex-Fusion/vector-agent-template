...
- **Boot guard:** the agent will not start serving the `custom` capability unless `SERVICE_TIMEOUT_MS` is comfortably inside your advertised SLA, specifically less than `ADVERT_MAX_PROCESSING_MS - 30000`. This is deliberate: a supplier whose own service budget can exceed what it advertised would claim jobs it can never submit in time, forfeiting its bond for nothing. Fix your `.env`, don't fight the guard.
